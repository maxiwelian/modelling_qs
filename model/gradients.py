...
class KFAC_Actor():
    def __init__(self,
        ...
        for w in model.trainable_weights:
            self.layers.append(w.name)
            self.m_aa[w.name] = tf.ones(w.shape)
            self.m_ss[w.name] = tf.ones(w.shape)

        self.should_center = True  # this is true by default as it is correc though we can change

        if conv_approx == 'mg':
            self.absorb_j = lambda x: tf.reshape(x, (-1, *x.shape[2:]))
            self.compute_normalization = lambda n_samples, conv_factor: n_samples * conv_factor
        elif conv_approx == 'ba':
            self.absorb_j = lambda x: tf.reduce_mean(x, axis=1)
    ...
    def extract_grads_and_a_and_s(self, model, inp, e_loc_centered, n_samples):
        ...
        grads[-1] = tf.reshape(grads[-1], (n_s, n_a))

        s_w = pre_activations[-1]
        pre_activations = pre_activations[:-1]
        ...
        for a, s, g, name in zip(activations, sensitivities, grads, self.layers):
            ...
            normalize = self.compute_normalization(n_samples, conv_factor)

            aa = tf.linalg.matmul(a, a, transpose_a=True) / normalize
            ss = tf.linalg.matmul(s, s, transpose_a=True) / normalize

            assert aa.shape[:-2] == ss.shape[:-2] == g.shape[:-2]
...
class KFAC():
    ...
    def compute_updates(self, grads, m_aa, m_ss, iteration):
        ...
        nat_grads = []
        for g, maa, mss, layer in zip(grads, m_aa, m_ss, self.layers):
            conv_factor = float(layer[-1])
            ...
            ng = self.compute_nat_grads(vals_a, vecs_a, vals_s, vecs_s, grads, decomp_damping) / conv_factor
